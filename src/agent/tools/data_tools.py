...
def _fetch_tushare_financial_report_fallback(stock_code: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warning("Tushare financial_report fallback failed for %s: %s", stock_code, exc)
        return {}

    report = {
        "report_date": _first_non_null(
            indicator.get("end_date"),
            income.get("end_date"),
            cashflow.get("end_date"),
        ),
        "ann_date": _first_non_null(
            indicator.get("ann_date"),
            income.get("ann_date"),
            cashflow.get("ann_date"),
        ),
        "revenue": income.get("revenue"),
        "net_profit_parent": income.get("n_income_attr_p"),
        "operating_cash_flow": cashflow.get("n_cashflow_act"),
        "roe": _first_non_null(indicator.get("roe"), indicator.get("roe_dt")),
        "revenue_yoy": indicator.get("or_yoy"),
        "net_profit_yoy": _first_non_null(
            indicator.get("netprofit_yoy"),
            indicator.get("dt_netprofit_yoy"),
        ),
        "gross_margin": indicator.get("grossprofit_margin"),
        "currency": "CNY",
        "source": "tushare:fina_indicator,income,cashflow",
    }
    return report if _is_non_empty_financial_report(report) else {}
...
def _handle_get_stock_info(stock_code: str) -> dict:
    ...
    if not _is_non_empty_financial_report(financial_report):
        fallback_report = _fetch_tushare_financial_report_fallback(stock_code)
        if fallback_report:
            financial_report = fallback_report
            tushare_fallback_used = True
            earnings["financial_report"] = fallback_report
            earnings_context = compact_context.get("earnings")
            if isinstance(earnings_context, dict):
                data = earnings_context.setdefault("data", {})
                if isinstance(data, dict):
                    data["financial_report"] = fallback_report
                earnings_context["status"] = earnings_context.get("status") or "partial"
    ...
    return {
        ...
        "fundamental_data_quality": {
            ...
            "has_financial_report": _is_non_empty_financial_report(financial_report),
            "financial_report_source": financial_report.get("source") if isinstance(financial_report, dict) else None,
            "tushare_fallback_used": tushare_fallback_used,
